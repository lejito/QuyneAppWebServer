  ...
  async verificarSaldoSuficiente(req, res) {
    try {
      ...
      const saldoSuficiente =
        verificarSaldoSuficiente[0][0].verificar_saldo_suficiente_bolsillo;
      ...
      if (saldoSuficiente != null) {
        ...
        res
          .status(200)
          .json(utils.warningResponse("No se encontró ningún bolsillo.", null));
      }
    } catch (error) {
      res
        .status(500)
    ...
    try {
      ...
      if (realizarCargaBolsillo[0].length > 0) {
        const movimiento = utils.convertSnakeToCamel(
          realizarCargaBolsillo[0][0]
        );

        res.status(200).json(
          utils.successResponse("Carga a bolsillo realizada correctamente.", {
            movimiento,
          })
        );
      } else {
        res
          .status(200)
          .json(
            utils.errorResponse("No se realizó la carga a bolsillo.", null)
          );
      }
    } catch (error) {
      res
          ...
          utils.successResponse(
            "Movimientos del bolsillo recuperados correctamente.",
            {
              movimientos: utils.convertSnakeToCamel(
                consultarUltimosMovimientosBolsillo[0]
              ),
            }
          )
        );
