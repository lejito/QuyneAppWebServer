...
class CuentasController {
  async consultarIdCuentaIdUsuarioAUX(idUsuario) {
    try {
      ...
      const idCuenta =
        consultarCodigoCuenta[0][0].consultar_codigo_cuenta_id_usuario;
  ...
  async consultarIdCuentaNumeroTelefonoAUX(numeroTelefono) {
    try {
      ...
      const idCuenta =
        consultarCodigoCuentaNumeroTelefono[0][0]
          .consultar_codigo_cuenta_numero_telefono;
  ...
  async consultarIdCuentaIdUsuario(req, res) {
    try {
      ...
      if (idCuenta != -1) {
        res.status(200).json(
          utils.successResponse("Id de cuenta recuperado correctamente.", {
            idCuenta,
          })
        );
      } else {
    ...
    try {
      ...
      if (activarSaldoOculto[0][0].activar_saldo_oculto) {
        res
          .status(200)
          .json(
            utils.successResponse("Saldo oculto activado correctamente.", {})
          );
      } else {
        res
          .status(200)
    ...
    try {
      ...
      if (desactivarSaldoOculto[0][0].desactivar_saldo_oculto) {
        res
          .status(200)
          .json(
            utils.successResponse("Saldo oculto desactivado correctamente.", {})
          );
      } else {
        res
          .status(200)
  ...
  async verificarExistenciaNumeroTelefono(req, res) {
    try {
      ...
      const cuentaExistente =
        verificarExistenciaCuentaNumeroTelefono[0][0]
          .verificar_existencia_cuenta_numero;
  ...
  async verificarSaldoSuficiente(req, res) {
    try {
      ...
      const saldoSuficiente =
        verificarSaldoSuficiente[0][0].verificar_saldo_suficiente;
      ...
      if (saldoSuficiente != null) {
        ...
        res
          .status(200)
          .json(utils.warningResponse("No se encontró ninguna cuenta.", null));
      }
    } catch (error) {
      res
        .status(500)
